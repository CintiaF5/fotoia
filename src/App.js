...
function App() {
  ...
  function ListaPessoas(props) {
    ...
    const listagemPessoas = pessoas.map((pessoa) =>
      <img key={pessoa.id} src={pessoa.urls[4][512]}
        tittle="Pessoa gerada via IA" alt="Pessoa gerada via IA" />
    )
    return (
      <>{listagemPessoas}</>
    )
  ...
  async function obtemFoto() {
    ...
    await fetch(url)
      ...
      .catch(function (error) {
        console.error('Houve um erro na requisição' + error.messege)
      })
    setCarregando(false)
  }
  ...
  return (
    <div className="app">
      ...
      <Robot />
      {carregando &&
        <img src={Carregando} tittle="Aguarde..." alt="Aguarde" width="50" />
      }
      <div className='linha'>
